...
function sizeLabel(s: BreedSize) {
  const m: Record<BreedSize, string> = {
    petit: 'Small',
    moyen: 'Medium',
    grand: 'Large',
  };
...
export default function BreedSearchEn({ breeds }: Props) {
  ...
  const fmtCAD = (n: number) => `$${new Intl.NumberFormat('en-CA').format(n)}`;
    ...
    <div>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          placeholder="Search breed, origin, health condition…"
      ...
      <div className="mb-3 flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide whitespace-nowrap">⚡ Max energy:</span>
        ...
        <div className="flex items-center gap-2 ml-auto">
          <label className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Sort:</label>
          <select value={sortBy} onChange={e => setSortBy(e.target.value as SortKey)}
            ...
            <option value="annualVetCost">Vet cost ↑</option>
      ...
      <div className="mb-4">
        ...
        {showAdv && (
          <div className="mt-3 grid gap-4 rounded-2xl border border-gray-100 bg-gray-50 p-4 sm:grid-cols-3">
            {/* Max grooming */}
            <div>
              <p className="mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wide">✂️ Max grooming</p>
              <div className="flex items-center gap-2">
            ...
            {/* Kids friendliness min */}
            <div>
              <p className="mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wide">👶 Kid-friendly min</p>
              <div className="flex items-center gap-2">
            ...
            {/* Max vet budget */}
            <div>
              <p className="mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wide">💵 Max vet budget/yr</p>
              <div className="flex items-center gap-2">
              ...
              <p className="mt-1 text-xs font-bold text-gray-700">
                {budgetMax >= COST_MAX_LIMIT ? 'Any budget' : `≤ ${fmtCAD(budgetMax)}/yr`}
              </p>
            </div>
          </div>
        )}
      ...
      <div className="mb-4 flex items-center justify-between">
        <p className="text-sm text-slate-400">
          <strong className="font-semibold text-gray-700">{filtered.length}</strong>{' '}
          breed{filtered.length !== 1 ? 's' : ''}
          {hasFilters ? ' matching your filters' : ' total'}
        </p>
        <div className="flex items-center gap-2">
          {hasFilters && (
          ...
          <button
            ...
            className="rounded-xl border border-gray-200 px-3 py-1.5 text-xs text-slate-500 hover:bg-gray-50 transition-colors"
            title="Copy link with active filters"
          >
            🔗 Share
          </button>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-gray-100 bg-gray-50 py-12 text-center">
          <p className="text-slate-500">No breeds match these filters.</p>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map(breed => {
            ...
            return (
              <a key={breed.slug} href={`/en/breeds/${breed.slug}`}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-slate-400">
                    {sizeLabel(breed.size)} · {breed.lifespan.min}–{breed.lifespan.max} yrs · {breed.origin}
                  </p>
                  ...
                  <p className="mt-1.5 text-xs text-slate-400">
                    Vet: <span className="font-medium text-gray-600">{fmtCAD(breed.annualVetCost.min)}–{fmtCAD(breed.annualVetCost.max)}/yr</span>
                  </p>
                </div>
              </a>
            );
